...
# %%
# Your code here:
tensor_1_1 = torch.arange(0, 9).reshape(3, 3)
tensor_1_1
...
negative_image = 255 - bear_tensor.clone()
#
# # Display your result
plt.figure(figsize=(8, 6))
plt.imshow(negative_image.numpy())
plt.axis("off")
plt.show()
...
# ## Problem 3.2
#
# Rotate the bear image by 180 degrees clockwise.
#
...
rotated_image = torch.flip(
    bear_tensor, dims=[0,1]
)
#
# # Display your result
plt.figure(figsize=(8, 6))
...
plt.title(f'rval = {rval:.3f}', fontsize=15)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
# No obvious relationship between reaction time and depression
...
plt.tight_layout()
plt.show()
# Strong relationship between depression and worthlessness self-reports
...
if language in production_averages:
    ...
    a = 26  # midpoint (age at which production is 50% of maximum)
    b = 0.25  # steepness (higher value = steeper curve)
    c = 700  # maximum value (asymptote)
...
if language in production_averages:
    ...
    a = 25  # Try different values
    b = 0.3  # Try different values
    c = 620  # Try different values
...
# %% [markdown]
# #### Exercise 1: Compute time-averaged responses by category
#
...
def compute_region_category_means(category_means_tensor):
    ...
    """
    # YOUR CODE HERE
    # Compute mean across neurons (dimension 1)
    return torch.mean(category_means_tensor, dim=1)
