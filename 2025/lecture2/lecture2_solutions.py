...
## YOUR CODE HERE ##
linear_model.b.data = torch.tensor(true_b)
...
## YOUR CODE HERE ##
depression_days_model = LinearModel()

# Set parameters by manual adjustment
# Try different values to find a good fit
## YOUR CODE HERE ##
depression_days_model.w.data = torch.tensor(0.5)  # Initial guess for slope
depression_days_model.b.data = torch.tensor(0.1)  # Initial guess for intercept

# Generate predictions
## YOUR CODE HERE ##
days_lost_pred = depression_days_model(x_depression)
...
# Create a fresh LinearModel instance
## YOUR CODE HERE ##
rt_days_model = LinearModel()

# Set parameters by manual adjustment
# Try different values to find a good fit
## YOUR CODE HERE ##
rt_days_model.w.data = torch.tensor(0.0)  # Initial guess for slope
rt_days_model.b.data = torch.tensor(1.0)  # Initial guess for intercept

# Generate predictions
## YOUR CODE HERE ##
days_lost_pred = rt_days_model(x_rt)
...
class LogisticModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.a = nn.Parameter(torch.tensor(0.0))  # midpoint
        self.b = nn.Parameter(torch.tensor(0.0))  # steepness
        self.c = nn.Parameter(torch.tensor(0.0))  # maximum value

    def forward(self, x):
        """
        Implement the logistic function: c / (1 + exp(-b * (x - a)))

        Args:
            x: Input tensor (age)

        Returns:
            y: Output tensor (predicted word production)
        """
...
english_model = LogisticModel()

# Set parameters by manual adjustment
# Adjust them to fit the data better
english_model.a.data = torch.tensor(25.0)  # midpoint
english_model.b.data = torch.tensor(0.25)  # steepness
english_model.c.data = torch.tensor(650.0)  # maximum value

# Generate predictions on a smoother grid for plotting
x_smooth = torch.linspace(min(ages), max(ages), 100)
y_pred = english_model(x_smooth)
...
# Create a logistic model instance
catalan_model = LogisticModel()

# Set parameters by manual adjustment
# Adjust them to fit the data better
catalan_model.a.data = torch.tensor(27.0)  # midpoint
catalan_model.b.data = torch.tensor(0.23)  # steepness
catalan_model.c.data = torch.tensor(700.0)  # maximum value

# Generate predictions on a smoother grid for plotting
x_smooth = torch.linspace(min(ages), max(ages), 100)
y_pred = catalan_model(x_smooth)
...
if language in production_averages:
    ...
    ## YOUR CODE HERE ##
    model.a.data = torch.tensor(25.0)  # midpoint
    model.b.data = torch.tensor(0.25)  # steepness
    model.c.data = torch.tensor(650.0)  # maximum value
...
plt.plot(
    ...
    "r-",
    linewidth=2,
    label=f"Model (a={model.a.item():.1f}, b={model.b.item():.3f}, c={model.c.item():.1f})",
)
...
## YOUR CODE HERE ##
parameter_sets = [
    {"name": "Poor Fit", "a": 38.0, "b": 0.05, "c": 300.0},  # Poor fit
    {"name": "Medium Fit", "a": 23.0, "b": 0.2, "c": 600.0},  # Medium fit
    {
        "name": "Good Fit",
        "a": 25.0,
        "b": 0.25,
        "c": 650.0,
    },  # Good fit (from previous example)
]
...
def absolute_loss(predictions, targets):
    """
    Compute the mean absolute error loss between predictions and targets.
...
if language in production_averages:
    ...
    for i, params in enumerate(parameter_sets):
        ...
        mae_value = absolute_loss(predictions, means_tensor)
        ...
        axs[i].set_title(
            f"{params['name']} (MAE: {mae_value.item():.1f}, MSE: {mse_value.item():.1f})",
            fontsize=14,
        )
        axs[i].set_xlabel("Age (months)", fontsize=12)
        axs[i].set_ylabel("Words Produced", fontsize=12)
...
# %%
def inverse_squared_loss(predictions, targets):
    """
    A "bad" loss function where better fits have higher loss values.
    The inverse of MSE: smaller errors lead to larger loss values.
    ...
    Returns:
        Inverse squared error loss
    """
    ## YOUR CODE HERE ##
    # Add a small epsilon to avoid division by zero
    epsilon = 1e-6
    return 100.0 / (torch.mean((predictions - targets) ** 2) + epsilon)
...
if language in production_averages:
    ...
    for i, params in enumerate(parameter_sets):
        ...
        # Calculate losses
        mse_value = squared_loss(predictions, means_tensor)
        bad_loss_value = inverse_squared_loss(predictions, means_tensor)
