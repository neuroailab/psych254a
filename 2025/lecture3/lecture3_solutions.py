...
print("3×2 matrix with all elements set to 5:")
print(matrix1)
...
print(f"1. u · v = {dot_uv.item()}")  # 2*4 + 3*0 + (-1)*5 = 8 - 5 = 3

# 2. Calculate the magnitudes
magnitude_u = torch.norm(u)
magnitude_v = torch.norm(v)
print(f"2. |u| = {magnitude_u.item():.4f}, |v| = {magnitude_v.item():.4f}")

# 3. Find the angle between vectors
cosine_theta = dot_uv / (magnitude_u * magnitude_v)
angle_rad = torch.acos(cosine_theta)
angle_deg = angle_rad * 180 / torch.pi
print(f"3. Angle between u and v: {angle_deg.item():.2f} degrees")

# 4. Check if vectors are orthogonal
is_orthogonal = torch.isclose(dot_uv, torch.tensor(0.0), atol=1e-6)
print(f"4. Are u and v orthogonal? {is_orthogonal.item()}")
...
# %%
# Define the matrices
P = torch.tensor([[2, 0], [3, 4], [1, -1]], dtype=torch.float32)  # 3x2 matrix

Q = torch.tensor([[5, 1, 2], [-2, 3, 7]], dtype=torch.float32)  # 2x3 matrix
...
# 1. Compute P × Q if valid
# P is 3x2 and Q is 2x3, so P × Q is valid (resulting in a 3x3 matrix)
PQ = P @ Q
print("\n1. P × Q (3×3) =")
print(PQ)

# 2. Compute Q × P if valid
# Q is 2x3 and P is 3x2, so Q × P is valid (resulting in a 2x2 matrix)
QP = Q @ P
print("\n2. Q × P (2×2) =")
print(QP)

# 3. Manually verify one element
# Verify PQ[0,0] = P[0,:] · Q[:,0]
row = P[0]  # [2, 0]
col = Q[:, 0]  # [5, -2]
manual_calc = torch.dot(row, col)
print(f"\n3. Manually verify PQ[0,0] = {row} · {col} = {manual_calc.item()}")
print(f"   Is this equal to PQ[0,0]? {manual_calc.item() == PQ[0,0].item()}")

# %%
# Define the matrices
P = torch.tensor([[2, 0], [3, 4], [1, -1]], dtype=torch.float32)  # 3x2 matrix

Q = torch.tensor([[5, 1, 2, 4], [-2, 3, 7, 10]], dtype=torch.float32)  # 2x3 matrix
...
# %%
# Define the matrix
M = torch.tensor([[3, -1, 2], [0, 4, 5]], dtype=torch.float32)  # 2x3 matrix
...
# 2. Compute N = MᵀM
N = M_T @ M  # This is a 3x3 matrix
print("\n2. N = MᵀM =")
print(N)

# 3. Check if N is symmetric
N_T = N.T
is_square = N.shape[0] == N.shape[1]
is_symmetric = torch.allclose(N, N_T)

print(f"\n3. Is N a square matrix? {is_square}")
print(f"   Is N symmetric (N = Nᵀ)? {is_symmetric}")
...
def plot_func_and_slopes(
    F,  #the funtion you want to plot
    dF, #the derivative of F, as a function
    xvals,
    title, start=-5, stop=5, start_delta=-1, stop_delta=1
):
...
plot_func_and_slopes(F, dF, xvals=[-2, 0, 1, 2], title="F(x) = x²")
...
def plot_function_with_concavity(F, #the original function
                                 dF,#the derivative function
                                 d2F, #the second derivative as a function
                                 title, x_range=(-4, 4)):
    """
...
# %%
# 1. Calculate the derivative of f(x) = x^3 - 3x + 1
...
print(f"f'(-1) = {slopes[0].item()}")
print(f"f'(0) = {slopes[1].item()}")
print(f"f'(1) = {slopes[2].item()}")

# 3. Interpret what the sign tells you about the function at these points
interpretations = [
    "Function is increasing",
    "Function is decreasing",
    "Function is at a critical point (not increasing or decreasing)",
]
...
for i, point in enumerate(points):
    ...
    if slope > 0:
        ...
        interp = interpretations[1]
    else:
        interp = interpretations[2]
    print(f"At x = {point.item()}, f'(x) = {slope}, so the {interp}.")

# Plot the function and its tangent lines for visualization
plot_func_and_slopes(F, dF, points, "f(x) = x³ - 3x + 1", -2, 2)
...
# %%
# 1. Taylor series of sin(x) around x = 0 up to 5th order
# sin(x) ≈ sin(0) + cos(0)(x) - sin(0)(x²/2!) - cos(0)(x³/3!) + sin(0)(x⁴/4!) + cos(0)(x⁵/5!)
# sin(x) ≈ 0 + 1(x) - 0(x²/2) - 1(x³/6) + 0(x⁴/24) + 1(x⁵/120)
# sin(x) ≈ x - x³/6 + x⁵/120

# 2. Compute the derivatives
f_sin = lambda x: torch.sin(x)
df_sin = lambda x: torch.cos(x)
d2f_sin = lambda x: -torch.sin(x)
d3f_sin = lambda x: -torch.cos(x)
d4f_sin = lambda x: torch.sin(x)
d5f_sin = lambda x: torch.cos(x)
...
print("Values of derivatives at x = 0:")
for i, val in enumerate(derivatives_at_0):
    print(f"f^({i})(0) = {val}")


# 4. Verify the Taylor series
...
x = torch.linspace(-np.pi, np.pi, 500)
y_sin = torch.sin(x)
y_taylor = taylor_sin(x, order=5)

plt.figure(figsize=(10, 6))
plt.plot(x, y_sin, "b-", label="sin(x)")
plt.plot(x, y_taylor, "r--", label="Taylor approximation (order 5)")
plt.xlabel("x")
plt.ylabel("y")
plt.title("sin(x) and its Taylor approximation")
plt.legend()
plt.grid(True)
plt.show()
...
# %%
# 1. Compute the gradient of f(x, y) = 3x^2 + 2xy - y^2
F_problem = lambda x, y: 3 * x**2 + 2 * x * y - y**2
dF_problem = lambda x, y: torch.tensor([6 * x + 2 * y, 2 * x - 2 * y])

# 2. Evaluate the gradient at (1, 2)
point = torch.tensor([1.0, 2.0])
grad_at_point = dF_problem(point[0], point[1])
print(f"∇f(1, 2) = {grad_at_point}")

# 3. Direction of steepest ascent
# The gradient points in the direction of steepest ascent
grad_magnitude = torch.sqrt(torch.sum(grad_at_point**2))
unit_vector = grad_at_point / grad_magnitude
print(f"Magnitude of gradient: {grad_magnitude.item():.4f}")
print(f"Unit vector in direction of steepest ascent: {unit_vector.tolist()}")

# 4. Plot the function and its gradient field
plot_function_and_gradient(
    F_problem,
    dF_problem,
    title="f(x,y) = 3x² + 2xy - y²",
    x_range=(-2, 2),
    y_range=(-2, 2),
)

# Mark the point (1, 2) on another plot
...
for i in range(X.shape[0]):
    for j in range(Y.shape[1]):
        Z[i, j] = F_problem(X[i, j], Y[i, j])

surf = ax1.plot_surface(X.numpy(), Y.numpy(), Z.numpy(), cmap=cm.coolwarm, alpha=0.8)
point_z = F_problem(point[0], point[1])
ax1.scatter(
    [point[0]], [point[1]], [point_z], color="black", s=100, label="Point (1, 2)"
)
ax1.set_xlabel("x")
ax1.set_ylabel("y")
ax1.set_zlabel("f(x,y)")
ax1.set_title("Function with Point (1, 2)")

# Contour plot with gradient at (1, 2)
ax2 = fig.add_subplot(2, 1, 2)
contour = ax2.contourf(X.numpy(), Y.numpy(), Z.numpy(), 20, cmap=cm.coolwarm)
ax2.scatter([point[0]], [point[1]], color="black", s=100)
ax2.quiver(
    [point[0]],
    [point[1]],
    [grad_at_point[0]],
    ...
    scale=10,
    width=0.01,
    label="Gradient at (1, 2)",
)
ax2.set_xlabel("x")
ax2.set_ylabel("y")
ax2.set_title("Contour Plot with Gradient at (1, 2)")
plt.colorbar(contour, ax=ax2)
ax2.legend()
